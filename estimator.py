    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                #print(body_part,center)
                centers[i] = center
                cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
                
            try:
                    #if (center[8] and center[9] and center[10] and center[11] and center[12] and center[13])==True:
                ankle_1_x=centers[10][0]
                ankle_1_y=centers[10][1]
                ankle_2_x=centers[13][0]
                ankle_2_y=centers[13][1] 
                knee_1_x=centers[9][0]
                knee_1_y=centers[9][1]
                knee_1_x=centers[12][0]
                knee_1_y=centers[12][1]
                B_1_x=centers[8][0]
                B_1_y=centers[8][1]
                B_2_x=centers[11][0]
                B_2_y=centers[11][1]
                #order,_=enumerate(common.CocoPairsRender)
                #print(order)
                if (centers[10][0] - centers[9][0]) !=0 and (centers[9][0] - centers[8][0]) !=0 and (centers[13][0] - centers[12][0]) !=0 and (centers[12][0] - centers[11][0]) !=0 :
                    slope_1=(centers[10][1] - centers[9][1]) / (centers[10][0] - centers[9][0])
                    slope_2=(centers[9][1] - centers[8][1]) / (centers[9][0] - centers[8][0])
                    knee_angle_1 = math.degrees(math.atan((slope_2 - slope_1) / (1 + slope_2 * slope_1)))
                    #print(knee_angle_1)
                    slope_1=(centers[13][1] - centers[12][1]) / (centers[13][0] - centers[12][0])
                    slope_2=(centers[12][1] - centers[11][1]) / (centers[12][0] - centers[11][0])
                    knee_angle_2 = math.degrees(math.atan((slope_2 - slope_1) / (1 + slope_2 * slope_1)))
                    #print(knee_angle_2)
                    if ankle_1_x > ankle_2_x:
                        Front_Knee_Angle = knee_angle_1
                        Back_Knee_Angle = knee_angle_2
                        print("Front Knee Angle : %f" % (abs(Front_Knee_Angle)))
                        print("Back Knee Angle : %f" % (abs(Back_Knee_Angle)))
                    else:
                        Front_Knee_Angle = knee_angle_2
                        Back_Knee_Angle = knee_angle_1
                        print("Front Knee Angle : %f" % (abs(Front_Knee_Angle)))
                        print("Back Knee Angle : %f" % (abs(Back_Knee_Angle)))
                    cv2.putText(npimg,"Angle of Front Knee: %f" % abs((Front_Knee_Angle)),(10, 30),  cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 0, 255), 2)
                    cv2.putText(npimg,"Angle of Back Knee: %f" % abs((Back_Knee_Angle)),(10, 50),  cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 0, 255), 2)
                    if abs(Front_Knee_Angle) > 20:
                        cv2.putText(npimg," INJURY PRONE ACTION",(10, 75),  cv2.FONT_HERSHEY_SIMPLEX, 0.6,(0, 0, 255), 2)
                        print(" INJURY PRONE ACTION")
                        cv2.putText(npimg," TRY TO STRAIGHTEN YOUR FRONT LEG",(10, 95),  cv2.FONT_HERSHEY_SIMPLEX, 0.6,(0, 0, 255), 2)
                        print(" TRY TO STRAIGHTEN YOUR FRONT LEG")
            except IndexError and KeyError:
                print(" Incomplete View! ")
                print(" Bowler needs to be completely visible ")
                pass
                cv2.putText(npimg,"Try Again with broader Action",(10, 30),  cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 0), 2)
            except ZeroDivisionError:
            	print(" Check action again with broader view ")
            	cv2.putText(npimg,"Try Again with broader Action",(10, 50),  cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 0), 2)
            	pass
            # draw line
            ...
            #cv2.imwrite('/home/haroon/tf-pose-estimation-master/images/check_{}.jpg'.format(time.time()-t_2),npimg)
            
                #point_1= centers[pair[0]]
                #point_2= centers[pair[1]]
                #slope_1=(centers[pair[1]][1] - centers[pair[0]][1]) / (centers[pair[1]][0] - centers[pair[0]][0])
                #tan_theta=abs((0))
